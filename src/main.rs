use dual_shock4_controller::gamepad::GamePad;
...
fn make_user_directory_and_config_file_and_get_divide_number(controller_mouse_path: String) -> i32 {
    ...
    match File::open(config_file_path.clone()) {
        ...
        Err(_error) => {
            ...
            fs::write(config_file_path.clone(), "20\n0x054c\n0x09cc")
                .expect("Could not write initial config file settings.");
        }
    };
...
fn handle_controller_input() {
    ...
    loop {
        let mut divide_number = make_user_directory_and_config_file_and_get_divide_number(
            home::home_dir().unwrap().to_str().unwrap().to_owned() + "/.controller_mouse",
        );
...
fn get_config(index: usize) -> String {
    return fs::read_to_string(
        home::home_dir().unwrap().to_str().unwrap().to_owned() + "/.controller_mouse/config.txt",
    )
    .unwrap()
    .as_str()
    .split("\n")
...
fn get_gamepad() -> GamePad {
    ...
    let device_info = DeviceInfo { vid: vid, pid: pid }; // vendor id, product id HID\VID_054C&PID_05C4\7&3869AC07&0&0000
